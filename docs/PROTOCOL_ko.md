...
* **요청**
  ...
  - `platform` (String, 필수): 엔티티 플랫폼 타입. 지원 목록: `sensor`, `binary_sensor`, `switch`, `number`, `select`, `button`
...
또한 **설정 → WebSocket Bridge → 게이트웨이 Subentry 삭제** 시, 해당 `gateway_id`의 기기·엔티티가 자동으로 정리됩니다.
